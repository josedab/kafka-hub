...
// Derive available where filters from actual data
const availableWheres = Array.from(new Set(errors.map((e) => e.where)));
const whereFilters: Array<ErrorEntry["where"] | "all"> = ["all", ...availableWheres];

function labelForWhere(value: ErrorEntry["where"] | "all") {
  return value === "all" ? "All" : value;
}

export function ErrorsFilterClient() {
  ...
  const [activeWhere, setActiveWhere] = useState<ErrorEntry["where"] | "all">("all");

  useEffect(() => {
    // URL navigation is the source of truth for back/forward changes.
